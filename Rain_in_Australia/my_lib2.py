...
def nan_info(data):
    ...
    for i in data.columns:
        ...
        nan_v = data[i].isna().sum()
        perc = round((nan_v/len(data[i]))*100, 2) # % NaN values for column
...
# data_string to datetime

def get_day_of_year(date_string):
    datetime_day = int(datetime.strptime(date_string, '%Y-%m-%d').strftime('%j'))
    return datetime_day

def get_month_of_year(date_string):
    datetime_month = int(datetime.strptime(date_string, '%Y-%m-%d').strftime('%m'))
    return datetime_month

def get_year(date_string):
    datetime_year = int(datetime.strptime(date_string, '%Y-%m-%d').strftime('%Y'))
